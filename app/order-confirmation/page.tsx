...
export default function OrderConfirmationPage() {
  ...
  const handleDownloadPackingSlip = () => {
    ...
    doc.setFontSize(10);
    doc.setTextColor(...orange);
    doc.text(`Order #${orderDetails.orderNumber}`, margin, 25);
    doc.setTextColor(200, 200, 200);
    doc.text(new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }), pageWidth - margin, 25, { align: 'right' });
    ...
    doc.setTextColor(...gray);
    doc.setFont('helvetica', 'normal');
    doc.text(`Event: ${orderDetails.event.date ? new Date(orderDetails.event.date + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : 'N/A'}`, margin, y);
    y += 5;
    doc.text(`Time: ${orderDetails.event.time || 'N/A'}`, margin, y);
    y += 5;
    doc.text(`Guests: ${orderDetails.headcount}`, margin, y);
    y += 5;
    doc.text(`Contact: ${orderDetails.contact.firstName} ${orderDetails.contact.lastName} — ${orderDetails.contact.phone}`, margin, y);
    y += 5;
    doc.text(`Delivery: ${orderDetails.delivery.address}, ${orderDetails.delivery.city}, ${orderDetails.delivery.state} ${orderDetails.delivery.zip}`, margin, y);
    y += 5;
    if (orderDetails.event.specialInstructions) {
      doc.text(`Notes: ${orderDetails.event.specialInstructions}`, margin, y);
      y += 5;
    }
    y += 6;
    ...
    doc.setTextColor(...dark);
    doc.setFont('helvetica', 'bold');
    doc.text(`Setup: ${orderDetails.event.setupRequired ? 'FULL SETUP REQUIRED' : 'DROP-OFF ONLY'}`, margin, y);
    y += 12;
    ...
    doc.save(`LexBetty-PackingSlip-${orderDetails.orderNumber}.pdf`);
  };
      ...
      <div className="container mx-auto px-4 py-8 sm:py-12 max-w-4xl">
        <div className="grid gap-6">
          ...
          <Card className="bg-white">
            <h2 className="font-oswald text-lg font-bold text-[#1A1A1A] mb-3 flex items-center gap-2">
              <svg className="w-5 h-5 text-[#E8621A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
              </svg>
              Kitchen Printouts
            </h2>
            <p className="text-sm text-gray-500 mb-4">Print documents for the kitchen team.</p>
            <div className="flex flex-col sm:flex-row gap-3">
              <Button variant="outline" onClick={handleDownloadPackingSlip} className="flex-1 flex items-center justify-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                </svg>
                Packing Slip (No Prices)
              </Button>
              <Button variant="outline" onClick={handleDownloadPDF} className="flex-1 flex items-center justify-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                </svg>
                Full Order (With Prices)
              </Button>
            </div>
          </Card>
        </div>
