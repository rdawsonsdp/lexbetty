...
function orderToPDFData(order: AdminOrder): OrderPDFData {
  return {
    ...
    event: {
      ...
      setupRequired: order.setup_required,
...
function AdminOrdersPage() {
  ...
  const SortArrow = ({ col }: { col: SortColumn }) => {
    ...
    return <span className="text-[#E8621A] ml-1">{sortDir === 'asc' ? '↑' : '↓'}</span>;
  };
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#E8621A]/30 focus:border-[#E8621A]"
          />
        </div>
                  ...
                  <tbody>
                    {orders.map((order) => (
                      <tr
                        ...
                        className="border-b border-gray-100 hover:bg-[#F5EDE0]/30 cursor-pointer transition-colors"
                      >
                        <td className="px-4 py-3 font-oswald font-bold text-[#E8621A]">
                          {order.order_number}
                        </td>
              ...
              <div className="md:hidden divide-y divide-gray-100">
                {orders.map((order) => (
                  <div
                    key={order.id}
                    onClick={() => { setSelectedOrder(order); setAdminNotes(order.admin_notes || ''); }}
                    className="p-4 hover:bg-[#F5EDE0]/30 cursor-pointer transition-colors"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-oswald font-bold text-[#E8621A]">{order.order_number}</span>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
                ...
                <select
                  ...
                  className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#E8621A]/30 disabled:opacity-50"
                >
              ...
              <div className="flex flex-wrap gap-2">
                {(selectedOrder.status === 'pending' || selectedOrder.status === 'invoiced') && (
                  <a
                    href={`/admin/orders/${selectedOrder.id}/edit`}
                    className="flex items-center gap-1.5 px-4 py-2 bg-[#E8621A] text-white text-sm font-semibold rounded-lg hover:bg-[#c8531a] transition-colors"
                  >
                ...
                {selectedOrder.status !== 'ready' && selectedOrder.status !== 'complete' && (
                  <button
                    onClick={() => handleStatusUpdate(selectedOrder.id, 'ready')}
                    disabled={updatingStatus}
                    className="flex items-center gap-1.5 px-4 py-2 bg-purple-600 text-white text-sm font-semibold rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-50"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                  ...
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <p className="font-medium text-[#1A1A1A]">{selectedOrder.setup_required ? 'Full' : 'Drop-off'}</p>
                  </div>
                </div>
                  ...
                  <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs font-semibold text-green-700 uppercase">Tax Exempt</p>
                      </div>
                      {selectedOrder.tax_exempt_certificate_url && (
                        <button
                          onClick={async () => {
                            try {
                              const res = await fetch(
                                `/api/admin/tax-certificate-url?path=${encodeURIComponent(selectedOrder.tax_exempt_certificate_url!)}`,
                                { headers: authHeaders() }
                              );
              ...
              {/* Admin Notes */}
              <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Admin Notes</h3>
                <textarea
                  value={adminNotes}
                  onChange={(e) => setAdminNotes(e.target.value)}
                  rows={3}
                  placeholder="Internal notes — e.g. No Tomato, Extra napkins, Customer called to confirm..."
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#E8621A]/30 resize-none"
                />
                <div className="flex items-center justify-between mt-2">
                  <p className="text-xs text-gray-400">
                    {adminNotes !== (selectedOrder.admin_notes || '') ? 'Unsaved changes' : ''}
                  </p>
                  <button
                    onClick={() => handleSaveNotes(selectedOrder.id)}
                    disabled={savingNotes || adminNotes === (selectedOrder.admin_notes || '')}
                    className="px-4 py-1.5 bg-[#1A1A1A] text-white text-xs font-semibold rounded-lg hover:bg-[#E8621A] transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                  >
              ...
              {/* Totals */}
              <div className="bg-[#F5EDE0] rounded-xl p-4 space-y-2">
                ...
                <div className="flex justify-between font-oswald font-bold text-lg pt-2 border-t border-[#E8621A]/30">
                  ...
                  <span className="text-[#E8621A]">{formatCurrency(selectedOrder.order_total)}</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-2.5 border-2 border-[#E8621A] text-[#E8621A] font-oswald font-bold rounded-lg hover:bg-[#E8621A] hover:text-white transition-colors text-sm"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
